      ...
      {showInviteModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 px-6 py-4 animate-in fade-in duration-300">
          <div className="relative w-full max-w-sm rounded-3xl bg-secondary border border-primary/30 p-6 shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[80vh]">
            ...
            {inviteSuccess && (
              <div className="mt-3 rounded-xl bg-emerald-500/20 border border-emerald-500/30 px-3 py-2 text-xs font-bold text-emerald-400 text-center animate-in fade-in duration-200">
                {inviteSuccess}
              </div>
            )}
                ...
                return nonParticipantFriends.map(friend => (
                  <div key={friend.id} className="flex items-center justify-between bg-card p-3 rounded-2xl border border-border shadow-soft">
                    <div className="flex items-center gap-2.5 min-w-0">
                      {friend.avatar_url ? (
                        <img src={friend.avatar_url} alt={friend.name} className="h-9 w-9 rounded-full object-cover shadow-sm shrink-0" />
                      ) : (
                ...
                <div className="relative">
                  <div className="h-16 w-16 rounded-full bg-card p-1 shadow-md ring-4 ring-secondary">
                    {formatted.avatar_url ? (
                      <img src={formatted.avatar_url} alt="Avatar" className="h-full w-full rounded-full object-cover" />
                    ) : (
...
const getFormattedProfile = (p: any) => {
  ...
  const name = username.includes('@') ? username.split('@')[0].split('.').map((n: string) => n.charAt(0).toUpperCase() + n.slice(1)).join(' ') : username;
  return {
    name, username, age, gender: p.gender || (charCodeSum % 2 === 0 ? 'Masculino' : 'Femenino'),
    location, bio, sports, emoji, gradient, rating: p.rating || 4.8, avatar_url: p.avatar_url,
    is_premium: p.is_premium || false, is_organizer: p.is_organizer || false,
  };
};
